...
def log_prior(theta):
   ...
   vc_prior_d = (theta[0:-4] > -400).all()
   vc_prior_u = (theta[0:-4] < 400).all()

   disk_prior = (theta[-4] > args.disk_scale - 1000) and (theta[-4] < args.disk_scale + 1000)
   vlos_prior = (theta[-3] > args.vlos_dispersion_scale - 1000) and (theta[-3] < args.vlos_dispersion_scale + 1000)

   r0_prior = (theta[-2] > 7800 and theta[-2] < 8500)
   z0_prior = (theta[-1] > -4 and theta[-1] < 30)

   if vc_prior_d and vc_prior_u and disk_prior and vlos_prior and r0_prior and z0_prior:
         return 0.0
   return -np.inf
...
theta_0[-4] = args.disk_scale
theta_0[-3] = args.vlos_dispersion_scale
theta_0[-2] = r_0
theta_0[-1] = z_0
