# ===========================================================================
#   Description: slightly change of corner function from corner module
#
# ===========================================================================
    ...
    **hist2d_kwargs
    ...
    for i, x in enumerate(xs):
        ...
        if smooth1d is None:
            ...
            # ================================================================
            # I have just modified line 238: I included "normed=True" in order
            # to visualise better when overplotting different 1D histogramns 
            # within same subplot
            # ================================================================
            if not np.all(np.isfinite(x)):
                continue
            #n, _, _ = ax.hist(x, bins=bins_1d, weights=weights, density=True,
            #                  range=np.sort(range[i]), linewidth=2., **hist_kwargs)
            bins_all,values_all = np.histogram(x, bins=bins_1d, normed=False,
                              range=np.sort(range[i]))
            weights_all=np.ones_like(x)/bins_all.max()
            n, _ = np.histogram(x, bins=bins_1d, weights=weights_all,
                              range=np.sort(range[i]))
            #n, _, _ = ax.hist(x, bins=bins_1d, weights=weights_all,
            #                  range=np.sort(range[i]), linewidth=2., **hist_kwargs)
...
def hist2d(x, y, bins=20, range=None, weights=None, levels=None, smooth=None,
    ...
    if plot_contours:
        ...
        ax.contour(X2, Y2, H2.T, V, alpha=1., linewidths=1.0, 
                   **contour_kwargs)
